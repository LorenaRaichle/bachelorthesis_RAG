# -*- coding: utf-8 -*-
"""
Analysis of results for Llama 2 Naive evaluation
"""
import os
import numpy as np
import pandas as pd
...
# Define file paths
input_file_path = "plots_Llama/eval_Llama.csv"
output_file_path_extracted = "plots_Llama/Llama_scores_extracted.csv"
output_file_path_summary = "plots_Llama/Llama_res.csv"
output_plots_dir = "plots_Llama/plots/"
...
df['bertscore_result'] = df['bertscore_result'].apply(ast.literal_eval)
...
numeric_columns = [
    'EM', 'F1', 'Sacrebleu_score', 'Sacrebleu_counts', 'Sacrebleu_bp', 'Sacrebleu_sys_len',
    'Sacrebleu_ref_len', 'rougeL_f1', 'rougeL_precision', 'rougeL_recall',
...
# df.to_csv("plots_Llama/Llama_res_with_outliers.csv", index=False)

# ---- Data Visualization ----
# Iterate over each numeric column to create box plots
for col in numeric_columns:
    plt.figure(figsize=(10, 6))
    sns.boxplot(x='category', y=col, data=df)
    plt.title(f'EVAL Llama: Distribution of {col} by Category')
    plt.xlabel('Category')
    plt.ylabel(col)
    plt.xticks(rotation=45)  # Rotate x-axis labels for better readability
    plt.grid(True)  # Show grid
    plt.tight_layout()  # Adjust layout to prevent clipping of labels
    plt.savefig(f'{output_plots_dir}{col}_boxplot_Llama.png')
    plt.show()
