# -*- coding: utf-8 -*-
"""
Analysis of results for the small-to-big comparison_approaches
"""
...
# Define file paths
input_file_path = "plots_smalltobig/check_df1.csv"
output_file_path_extracted = "plots_smalltobig/STB_scores_extracted.csv"
output_file_path_summary = "plots_smalltobig/STB_res.csv"
output_plots_dir = "plots_smalltobig/plots/"
...
# Display category counts and first row
...
def extract_sacrebleu_values(row):
    ...
    try:
        ...
        print(f"Error parsing sacrebleu_score: {e}")
...
# Optionally save the DataFrame with outliers
# df.to_csv("plots_smalltobig/STB_res_with_outliers.csv", index=False)
...
for col in numeric_columns:
    ...
    plt.title(f'EVAL STB: Distribution of {col} by Category')
    plt.xlabel('Category')
    plt.ylabel(col)
    plt.xticks(rotation=45)  # Rotate x-axis labels for better readability
    plt.grid(True)  # Show grid
    plt.tight_layout()  # Adjust layout to prevent clipping of labels
    plt.savefig(f'{output_plots_dir}{col}_boxplot_STB.png')
    plt.show()
